...
namespace Paho.Controllers
{
    ...
    public class ExportarController : ControllerBase
    {
                    ...
                    using (var excelPackage = new ExcelPackage(fs))
                    {
                        ...
                        else if (reportTemplate.ToUpper() == "R4")
                        {
                            ...
                            for (int i = 0; i <= contador; i++)
                            {
                                ...
                                AppendDataToExcel_R4(Languaje_, CountryID_, RegionID_, null, HospitalID_, Month, SE, StartDate, EndDate, excelWorkBook, "R4", (i > 0 ? (6 + (57 * i)) : 6), 1, excelWs_VIRUSES_IRAG.Index, false, ReportCountry, YearEnd_report, YearEnd_report, Surv, Inusual, AreaID_, Sentinel);
                                if (i > 0)
                                {
        ...
        private static void AppendDataToExcel_R2_SeveralYears(string languaje_, int countryId, int? regionId, int? year, int? hospitalId, int? month, int? se, DateTime? startDate, DateTime? endDate, ExcelWorkbook excelWorkBook, string storedProcedure, int startRow, int startColumn, int sheet, bool? insert_row, int? ReportCountry, int? YearFrom, int? YearTo, int? Surv, bool? SurvInusual, int? AreaId)               //#### CAFQ
        {
            ...
            //var zzz = excelWorksheet.Drawings[1];             // Un chart especifico
            ...
            int? YearFrom, int? YearTo, int? Surv, bool? SurvInusual, int? AreaId, int? Sentinel = null)               //#### CAFQ
        {
            ...
            bbbb(excelWorksheet, wsPara, wsTemp, nAnios, colW, row, col, srTA, startColumn, 6, 9, startRow - 3, widthYear_TA, heightYear_WK, nCoWiCopy, true);
            ...
            bbbb(excelWorksheet, wsPara, wsTemp, nAnios, colW, row, col, srTA, startColumn, 0, height2_TA, srTA - 2, widthYear_TA, heightYear_WK, nCoWiCopy, true);
            ...
            bbbb(excelWorksheet, wsPara, wsTemp, nAnios, colW, row, col, srTA, startColumn, 0, height2_TA, srTA - 2, widthYear_TA, heightYear_WK, nCoWiCopy, false);
            ...
            for (nI = 1; nI <= nAnios; ++nI)
            {
                ...
                AppendDataToExcel(languaje_, countryId, regionId, year, hospitalId, month, se, startDate, endDate, excelWorkBook,
                    ...
                    AreaId, colT2.ToString(), Sentinel);
            }
        ...
        private static void bbbb(ExcelWorksheet wsTabl, ExcelWorksheet wsPara, ExcelWorksheet wsWork, int nAnios, int nCoWi, int rowS, int colS,
                ...
                using (var command = new SqlCommand("select * from ReportCountryConfig where ReportCountryID = @ReportCountryID", con))
                {
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            if (YearFrom > 0)
                                labelYear += SgetMsg("msgViewExportarLabelFrom", countryId, languaje_) + " " + YearFrom + " ";

                            if (YearTo > 0)
                                labelYear += SgetMsg("msgViewExportarLabelTo", countryId, languaje_) + " " + YearTo + " ";
